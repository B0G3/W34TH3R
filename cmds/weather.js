...
const execFunction = async (bot, message, args) => {
	let result;
	if (!args[0]) {
		const location = await dataUtil.getUserLocation(message.author.id);
		if (!location) {message.channel.send({ content: getPhrase(message.guild, 'CMD_WEATHER_ERROR_1') });}
		else {
			const _args = [location.lat, location.lon];
	...
	if (!result.data || result.data.cod != 200) {
		...
		const embed = await embedUtil.weatherEmbed(message, result.data);
